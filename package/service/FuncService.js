...
let FuncService = class FuncService {
    ...
    createFunc(moduleToken, name) {
        return __awaiter(this, void 0, void 0, function* () {
            let module = yield this.moduleRepository.findOneById(moduleToken);
            if (!module) {
                throw new common_1.HttpException('指定模块token=' + moduleToken + '不存在', 415);
            }
            let exist = yield this.funcRepository.findOne({ name, moduleToken });
            if (exist) {
                throw new common_1.HttpException('指定模块token=' + moduleToken + '下，指定名称name=' + name + '功能已经存在', 416);
            }
            let func = this.funcRepository.create({ name, module });
            try {
    ...
    updateFunc(id, name) {
        return __awaiter(this, void 0, void 0, function* () {
            let func = yield this.funcRepository.findOneById(id);
            if (!func) {
                throw new common_1.HttpException('指定id=' + id + '功能不存在', 417);
            }
            if (name !== func.name) {
                let exist = yield this.funcRepository.findOne({ name, moduleToken: func.moduleToken });
                if (exist) {
    ...
    setPermissions(id, permissionIds) {
        return __awaiter(this, void 0, void 0, function* () {
            let func = yield this.funcRepository.findOneById(id, { relations: ['permissions'] });
            if (!func) {
                throw new common_1.HttpException('指定id=' + id + '功能不存在', 417);
            }
            let pers = yield this.permissionRepository.findByIds(permissionIds, { relations: ['module'] });
