...
let RoleService = class RoleService {
    ...
    createRole(moduleToken, name, score) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let exist = yield this.roleRepository.findOne({ name, moduleToken });
            if (exist) {
                throw new common_1.HttpException('指定模块token=' + moduleToken + '下，指定名称name=' + name + '角色已经存在', 420);
            }
            let role = this.roleRepository.create({ name, score, module });
            try {
    ...
    updateRole(id, name, score) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (name !== role.name) {
                let exist = yield this.roleRepository.findOne({ name, moduleToken: role.moduleToken });
                if (exist) {
    ...
    setFuncs(id, funcIds) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let funcs = yield this.funcRepository.findByIds(funcIds);
            funcIds.forEach(funcId => {
                let find = funcs.find(func => {
